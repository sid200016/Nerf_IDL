...
def render(
    ...
    if hasattr(cfg, 'training') and hasattr(cfg.training, 'checkpoint_path') and len(cfg.training.checkpoint_path) > 0:
        checkpoint_path = os.path.join(
            hydra.utils.get_original_cwd(),
            cfg.training.checkpoint_path
        )
    elif hasattr(cfg, 'checkpoint_path') and len(cfg.checkpoint_path) > 0:
        checkpoint_path = os.path.join(
            hydra.utils.get_original_cwd(),
            cfg.checkpoint_path
        )
    ...
    if checkpoint_path and os.path.isfile(checkpoint_path):
        ...
        print("Warning: No checkpoint path specified. Using untrained model.")
...
def main(cfg: DictConfig):
    ...
    if cfg.type == 'train_nerf':
        ...
        raise ValueError(f"Unknown type: {cfg.type}. Supported types: 'train_nerf', 'render'")
